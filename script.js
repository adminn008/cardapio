...
const URL_API = "https://script.google.com/macros/s/AKfycbzfzTnNpgAZcICqpbGwy8i7XbDG8Tea75L-r-aou7DpaeaEdmarv5oN9xf95imY9Fft/exec";

// Função para enviar o pedido para a planilha
document.getElementById('form-pedido')?.addEventListener('submit', function(e) {
  ...
  // Exemplo de captura dos dados (ajuste os nomes dos campos se necessário)
  const dadosPedido = {
    action: "novo_pedido",
    id: Math.floor(1000 + Math.random() * 9000),
    cliente: document.getElementById('nome').value,
    endereco: document.getElementById('endereco').value,
    itens: obterResumoItens(), // Sua função que monta a lista de texto dos itens
    total: parseFloat(document.getElementById('total-txt').textContent.replace('R$', '').replace(',', '.').trim()) || 0,
    pagamento: document.getElementById('pagamento').value,
    taxa: parseFloat(document.getElementById('taxa-txt').textContent.replace('R$', '').replace(',', '.').trim()) || 0,
  ...
  fetch(URL_API, {
    ...
    btnSubmit.innerHTML = "🚀 Confirmar & Enviar no WhatsApp";
    irParaEtapa('success');
  }).catch(err => {
